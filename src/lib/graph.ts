...
const GROUP_COLORS: Record<string, string> = {
  socmint: '#ff5d8f', sigint: '#ffd166', geoint: '#06d6a0', breach: '#ef476f',
  web: '#118ab2', darkweb: '#8338ec', people: '#fb8500', threat: '#e63946',
  reference: '#8ecae6', general: '#adb5bd',
};
const STATUS_RING: Record<string, string> = {
  active: '#3ddc97', dead: '#6c757d', risky: '#ff3b3b', unchecked: '#cbd5e1',
};
const color = (g: string) => GROUP_COLORS[g] ?? GROUP_COLORS.general;

interface SearchHit { type: 'category' | 'item'; category: Category; item?: Item }
...
export class AtlasGraph {
  ...
  constructor(svgEl: SVGSVGElement, feed: OsintFeed, onSelect: (n: GraphNode | null) => void) {
    ...
    this.sim = d3.forceSimulation<GraphNode, GraphLink>()
      .force('link', d3.forceLink<GraphNode, GraphLink>().id((d) => d.id).distance((l) => (l.kind === 'group' ? 160 : 60)).strength(0.6))
      .force('charge', d3.forceManyBody().strength((d) => ((d as GraphNode).kind === 'item' ? -40 : -260)))
      .force('collide', d3.forceCollide<GraphNode>().radius((d) => this.radius(d) + 6))
  ...
  private collapse(parentId: string): void {
    ...
    const walk = (pid: string) => {
      for (const n of this.nodes) if (n.parentId === pid && !remove.has(n.id)) { remove.add(n.id); walk(n.id); }
    };
    walk(parentId);
    this.nodes = this.nodes.filter((n) => !remove.has(n.id));
    this.links = this.links.filter((l) => !remove.has(idOf(l.target)));
    for (const id of remove) { this.expanded.delete(id); this.offsets.delete(id); }
  ...
  private childrenOf(parent: GraphNode): GraphNode[] {
    if (parent.kind === 'group') {
      return this.feed.categories
        ...
        .map((c) => ({ id: `cat:${c.slug}`, label: c.category, kind: 'category', group: c.group, count: c.items.length }));
    }
  ...
  private expand(parentId: string): void {
    ...
    if (end < kids.length) {
      ...
      this.nodes.push({ id: moreId, label: `+${kids.length - end} more`, kind: 'more', group: parent.group, parentId, orbitR: r, orbitA: a });
  ...
  private restart(): void {
    const link = this.linkLayer.selectAll<SVGLineElement, GraphLink>('line')
      .data(this.links, (d) => `${idOf(d.source)}->${idOf(d.target)}`);
    link.exit().remove();
    link.enter().append('line').attr('class', (d) => `link ${d.kind}`);

    const node = this.nodeLayer.selectAll<SVGGElement, GraphNode>('g.node')
      .data(this.nodes, (d) => d.id);
    node.exit().remove();
    const enter = node.enter().append('g').attr('class', (d) => `node ${d.kind}`);
    enter.append('circle')
    ...
    enter.on('click', (e, d) => {
      e.stopPropagation();
      if (d.kind === 'more') { this.expand(d.parentId!); return; }
      this.onSelect(d);
      if (d.kind === 'group' || d.kind === 'category') this.toggle(d.id);
    });
    enter.on('dblclick', (_e, d) => { if (d.url) window.open(d.url, '_blank', 'noopener'); });
  ...
  private tick(): void {
    ...
    for (const n of this.nodes) {
      if ((n.kind === 'item' || n.kind === 'more') && n.parentId && n.orbitR != null && n.fx == null) {
        ...
        if (p) { n.x = (p.x ?? 0) + Math.cos(n.orbitA!) * n.orbitR; n.y = (p.y ?? 0) + Math.sin(n.orbitA!) * n.orbitR; }
      }
    }
    this.linkLayer.selectAll<SVGLineElement, GraphLink>('line')
      .attr('x1', (d) => (d.source as GraphNode).x ?? 0)
      .attr('y1', (d) => (d.source as GraphNode).y ?? 0)
      .attr('x2', (d) => (d.target as GraphNode).x ?? 0)
      .attr('y2', (d) => (d.target as GraphNode).y ?? 0);
    this.nodeLayer.selectAll<SVGGElement, GraphNode>('g.node')
      .attr('transform', (d) => `translate(${d.x ?? 0},${d.y ?? 0})`);
  }

  private drag() {
    return d3.drag<SVGGElement, GraphNode>()
      .on('start', (e, d) => { if (!e.active) this.sim.alphaTarget(0.2).restart(); d.fx = d.x; d.fy = d.y; })
  ...
  search(query: string): void {
    ...
    for (const c of this.feed.categories) {
      ...
      groups.add(c.group);
      this.nodes.push({ id: `cat:${c.slug}`, label: c.category, kind: 'category', group: c.group, count: c.items.length, parentId: `group:${c.group}` });
    }
    for (const g of groups) {
      this.nodes.unshift({ id: `group:${g}`, label: g, kind: 'group', group: g });
      for (const c of this.feed.categories) if (c.group === g && slugs.has(c.slug))
  ...
  filterGroups(active: Set<string>): void {
    this.nodeLayer.selectAll<SVGGElement, GraphNode>('g.node')
      .style('display', (d) => (active.size === 0 || active.has(d.group) ? null : 'none'));
    this.linkLayer.selectAll<SVGLineElement, GraphLink>('line')
      .style('display', (d) => {
        const g = (d.target as GraphNode).group;
        return active.size === 0 || active.has(g) ? null : 'none';
      });
  }
}
